...
class Photo:
    def __init__(self, path):
        ...
        except IOError: # Manejo de errores: 
            messagebox.showerror("Error", f"Revise la ruta: {path}")
            raise
...
class Galeria:
    def __init__(self, root):
        ...
        # Creo un contenedor (frame) para mi listbox - albumes
        self.listbox_frame = tk.Frame(root, width=200, bg="gray")
        self.listbox_frame.pack(side=tk.LEFT, fill=tk.Y)

        # Creando listbox_frame
        self.album_listbox = tk.Listbox(self.listbox_frame, width=25)
        self.album_listbox.pack(side=tk.LEFT, fill=tk.Y, expand=True)

        # Creo el canvas para mis fotos
    ...
    def load_photo(self):
        if not self.current_album:
            messagebox.showwarning("No hay álbum", "Por favor seleccione o agregue un álbum primero")
            return
    ...
    def show_photos(self):
        ...
        # Tamaño de las imagenes y margen
        thumbnail_size = 150
        margin = 10
